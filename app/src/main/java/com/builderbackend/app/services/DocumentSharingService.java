...
public class DocumentSharingService {
    ...
    private void createDocumentSharingNotification(DocumentSharingDTO documentSharingDTO, String documentSharingId) {
        try {
            notificationService.createNotification(documentSharingDTO.getProjectId(), documentSharingId, EventType.EVENT_TYPE_4);
        } catch (Exception e) {
            ...
            if(folder.isPresent()){
                ...
                if (folderDTO.getDocuments().size() > 0) {
                    for (DocumentSharingDTO documentSharingDTO : folderDTO.getDocuments()) {
                        ...
                        notificationService.modifyNotificationViewedToTrueForDocId(folderDTO.getProjectId(), documentSharingDTO.getDocumentSharingId() ,EventType.EVENT_TYPE_10);    
                    }
                }
            ...
            if(folder.size() == 1){
                ...
                if (folderDTO.getDocuments().size() > 0) {
                    for (DocumentSharingDTO documentSharingDTO : folderDTO.getDocuments()) {
                        ...
                        notificationService.modifyNotificationViewedToTrueForDocId(projectId, documentSharingDTO.getDocumentSharingId() ,EventType.EVENT_TYPE_10);        
                    }
                }
    ...
    // this method will be used to delete a folder and its content, it uses recursion to get through all of its subfolders.. 
    // not sure I trust this yet since we dont have a backup plan if this fails mid way through
    // we also should somehow check for any risks of infinite recursion
    // we will keep these methods commented out for now
    // maybe change the delete methods to return booleans
    /*
    @Transactional
    public void deleteFolder(String folderId) {
        ...
        for (DocumentSharing document : folder.getDocuments()) {
            deleteDocumentContents(document);
        }
    ...
    private void deleteDocumentContents(DocumentSharing document) {
        List<FileInfo> fileInfoList = fileInfoRepository.findByFeatureId(document.getDocumentSharingId());
        ...
        documentSharingRepository.deleteById(document.getDocumentSharingId());
    }
    ...
    public void deleteDocument(String documentSharingId) throws Exception{
        ...
        notificationService.deleteNotificationForDocId(folder.get().getProject().getProjectId(), documentSharingId, EventType.EVENT_TYPE_4);
    ...
    private void deleteFileWithPermissions(String fileUrl) throws Exception {
        ...
        if (Files.isWritable(pathToFile)) {
            ...
            try {
                Process process = Runtime.getRuntime().exec("sudo rm " + "/moonlightUploads/"+fileUrl);
                process.waitFor();
            } catch (Exception e) {
